...
public class ProductDAOimpl implements ProductDAO {
	...
	public List<Product> getNewestProducts(Integer numberOfNewestProducts) {
		Session session = sessionFactory.openSession();
		try {
			Query<Product> query = session.createQuery("from Product p order by p.createDate desc");
	...
	public List<Product> getFeatureProducts() {
		Session session = sessionFactory.openSession();
		try {
			Query<Product> query = session.createQuery("from Product p where p.features = 1 order by p.createDate desc");
	...
	public List<Product> getRelatedProducts(Integer categoryId) {
		Session session = sessionFactory.openSession();
		try {
			Query<Product> query = session.createQuery("from Product p where p.categoryId = :categoryId order by p.createDate desc");
			query.setParameter("categoryId", categoryId);
			List<Product> result = query.getResultList();
			return result;
		} catch (Exception e) {
